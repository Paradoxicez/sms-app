...
---
*Last updated: 2026-04-25 after Phase 20 completion (Cameras UX bulk actions, maintenance toggle, copy ID, expressive status). 22/22 locked decisions implemented, 5/5 human UAT passed, 2 out-of-scope gaps fixed during UAT (tooltip z-index, Projects-page bulk wiring). 189 phase-20 tests + 478 web suite + 621 API suite all green. 22 pre-existing API test failures (status/dashboard mock drift + seed unique-constraint collision) also repaired during regression gate.*
