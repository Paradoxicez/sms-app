...
1. `pnpm --filter @sms-platform/api db:reset` — drops the dev DB, replays migration history (`prisma migrate reset --force --skip-seed`), and regenerates the Prisma client. Phase 23 (DEBT-05) replaced `db:push` with this — the migration history is now the source of truth. For schema changes, run `pnpm --filter @sms-platform/api db:migrate -- --name <change>` to produce a new migration directory; do NOT edit existing migrations or fall back to `db:push`. Both `db:reset` and `db:migrate` auto-load `.env` and override `DATABASE_URL` to `$DATABASE_URL_MIGRATE` (privileged user with DDL) — required because the runtime `app_user` lacks `CREATE/DROP TABLE` privileges.
